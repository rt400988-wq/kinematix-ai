...
import { ExerciseAnalyzer, CalibrationController, type ExerciseMode, type FormAlert } from "@/lib/exercise-analysis"
 ...
 *     is scored. See lib/exercise-analysis/calibration.ts CalibrationController.
...
export default function Page() {
  ...
  // count-the-movement) don't tangle — see module note in lib/exercise-analysis/analyzer.ts.
  ...
  // IMPORTANT: clearFormAlertTimer, flashFormAlert, and flashAndLogFormAlert
  // are called from handlePoseLandmarks below, which is a useCallback with
  // an EMPTY dependency array (stable identity, never recreated — see its
  // own comment for why). That means handlePoseLandmarks permanently holds
  // the version of these functions that existed when it was first created,
  // not whatever version exists on a later render. This is safe ONLY
  // because none of the three closes over actual reactive state — they only
  // touch refs (formAlertTimeoutRef, fbId) and setState setters, both of
  // which are stable identities regardless of which render's closure reads
  // them. If you edit any of these to read a state variable (mode, phase,
  // etc.) directly, route it through a ref the same way modeRef/phaseRef do
  // below — otherwise handlePoseLandmarks will silently use a stale value
  // forever, since it never re-renders to pick up a fresh closure.
  ...
  const handlePoseLandmarks = useCallback((landmarks: NormalizedLandmark[] | undefined, timestampMs: number) => {
    ...
    if (currentMode === "Plank") {
      ...
      // fields in lib/exercise-analysis/types.ts for why this is a separate path.
  ...
  // see lib/exercise-analysis/analyzer.ts. Labeling it "Strikes" instead of "Reps"
